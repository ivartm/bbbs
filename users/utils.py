class StaffRequiredAdminMixin(object):
    ...
    def has_view_permission(self, request, obj=None):
        if not request.user.is_anonymous:
            return request.user.profile.is_admin or request.user.profile.is_moderator_gen
        return self.check_perm(request.user)
...
class AdminAndModerMixin(object):
        ...
        def check_perm(self, user_obj):
            if user_obj.is_anonymous:
                return False
            if user_obj.profile.is_admin or user_obj.is_superuser or user_obj.profile.is_moderator_gen:
                return True
            return False
